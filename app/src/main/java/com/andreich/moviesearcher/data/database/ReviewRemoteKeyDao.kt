...
interface ReviewRemoteKeyDao {
    ...
    @Query("SELECT * FROM review_remote_key WHERE valueId = :id AND valueType = :type")
    suspend fun getRemoteKeyByValueID(id: Int, type: KClass<out Entities>): ReviewRemoteKeyEntity?

    @Query("DELETE FROM review_remote_key WHERE valueType = :type")
    suspend fun clearRemoteKeys(type: KClass<out Entities>)

    @Query("SELECT created_at FROM review_remote_key WHERE valueType = :type ORDER BY created_at DESC LIMIT 1")
